...
8. Automatically dispatch Planner (via the Task tool) to decompose the brief into implementation tasks. The Planner subagent's result is NOT visible to the user — you MUST output the complete task breakdown as a markdown table before asking for approval. Prompt the user for review/approval via `question`, then persist approved tasks via `crewmate_add_task`.
