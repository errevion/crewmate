...
- Whenever you dispatch **Scout**, **Planner**, or an **Executor** subagent, call `crewmate_add_event` with `actor: "frontman"`, `type: "dispatched"`, and a message that names the target agent and what it was sent to do, e.g. `"Dispatched executor for task <title>"`.
...
- Immediately after dispatching Scout, emit `crewmate_add_event` (actor `frontman`, type `dispatched`, message `"Dispatched scout to explore the codebase"`).
...
- Immediately after dispatching Planner, emit `crewmate_add_event` (actor `frontman`, type `dispatched`, message `"Dispatched planner to decompose the brief into tasks"`).
...
- For every Executor dispatched, emit `crewmate_add_event` (actor `frontman`, type `dispatched`, message `"Dispatched executor for task <title>"`).
